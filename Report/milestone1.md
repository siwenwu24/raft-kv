...
Raft consensus layer (hashicorp/raft) — handles leader election, log replication, and snapshotting
FSM (Finite State Machine) — a mutex-protected in-memory map that applies committed log entries
...
A clear, minimal Raft KV implementation that others can study to understand consensus internals without the complexity of production systems like etcd.
